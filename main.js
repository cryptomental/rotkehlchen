...
const createPyProc = () => {
    ...
    if (guessPackaged()) {
        ...
        pyProc = require('child_process').execFile(executable, ["--zerorpc-port", port, "--logfromothermodules", "--loglevel", "debug", "--output", "output.log"]);
    } else {
        pyProc = require('child_process').spawn('python', ["-m", "rotkehlchen", "--zerorpc-port", port,"--logfromothermodules", "--loglevel", "debug", "--output", "output.log"]);
    }
